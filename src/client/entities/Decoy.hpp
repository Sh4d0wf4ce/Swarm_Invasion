...
public:
    Decoy(std::uint32_t id, const sf::Vector2f& startPos, float maxHp = Config::VANGUARD_DECOY_HP)
        : Entity(id, startPos) {
        m_maxHp = maxHp;
        m_hp = maxHp;
    ...
    void update(sf::Time deltaTime, const std::shared_ptr<MapGenerator>& map) override {
        ...
        if (m_isExploding) {
            ...
            if (progress <= 1.0f) {
                ...
                float currentRadius = Config::VANGUARD_DECOY_EXPLOSION_RADIUS * easeOut;
    ...
    bool isFinished() const {
        return m_isExploding && (m_explosionTimer >= m_maxExplosionTime);
    }
